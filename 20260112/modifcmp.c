...
void print_files_newer_than(char* dirpath, time_t mtime) {
	DIR *dir = opendir(dirpath);
	if(!dir) return;

	struct dirent* dent = readdir(dir);
	printf("Modificati di recenti nella directory: %s\n", dirpath);
	while(dent != NULL) {
...
int main(int argc, char* argv[]) {
	switch (argc) {
		case 2: {
			struct stat st;
			stat(argv[1], &st);
			print_files_newer_than(".", st.st_mtime);
			break;
		}
		case 3: { 
			struct stat st1, st2;			
			...
			if(S_ISDIR(st2.st_mode)) {
				print_files_newer_than(argv[2], st1.st_mtime);
